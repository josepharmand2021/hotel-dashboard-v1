...
export type LedgerRow = {
  ...
  shareholder_id?: number | null;     // jika view join ke expenses/shareholders
...
/** -------- Top Up (PT / RAB) --------
 *  Gunakan satu fungsi ini. Jika source='RAB', wajib kirim shareholder_id.
 *  transfer_subcategory_id = id subkategori "Transfer Kas Kecil"
 */
export async function addTopup(payload: {
  ...
  note?: string | null;
  bank_account_id?: number | null; // opsional: disimpan di petty_cash_txns
  ref_no?: string | null;          // opsional
}) {
  const {
    source,
    cashbox_id,
  ...
  if (source === 'RAB') {
    ...
    // Lengkapi bank_account_id/ref_no pada row ledger hasil RPC (jika diisi pada form)
    const topupId = Array.isArray(data) ? data[0]?.topup_txn_id : (data as any)?.topup_txn_id;
    if (topupId && (bank_account_id || ref_no || note)) {
...
/** -------- Settlement (setor kembali ke PT/RAB) --------
 *  Ini hanya pergerakan kas kecil; tidak membuat baris expenses.
 */
