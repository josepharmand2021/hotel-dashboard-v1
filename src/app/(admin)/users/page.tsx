...
export default async function AdminUsersPage({
  ...
  const { data: list, error: listErr } = await admin.auth.admin.listUsers({ page, perPage } as any);
      ...
      <div className="mt-6 space-y-6">
        ...
        <section className="rounded-2xl border bg-card text-card-foreground shadow-sm">
          <div className="p-4 border-b">
            ...
            <p className="text-xs text-muted-foreground mt-1">Invite a user or set a password directly.</p>
          </div>
            ...
            <form action={createUserAndAssign} className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="md:col-span-1 flex items-end justify-between gap-3">
                <div className="flex-1">
                  ...
                  <select id="roleCode" name="roleCode" defaultValue="viewer" className="border rounded-md w-full h-9 px-3 text-sm">
                    <option value="viewer">viewer</option>
                    <option value="admin">admin</option>
                    <option value="superadmin">superadmin</option>
                  </select>
                </div>
                <button className="bg-black text-white h-9 px-4 rounded-md text-sm">Create</button>
              </div>
            </form>
          </div>
        </section>
        ...
        <section className="rounded-2xl border bg-card text-card-foreground shadow-sm mt-6">
          <div className="p-4 border-b flex items-center justify-between gap-3">
            ...
            <form className="flex items-center gap-2" method="GET" action="/users">
              ...
              <select name="role" defaultValue={roleFilter ?? 'all'} className="border rounded-md h-9 px-3 text-sm">
              ...
              <a href="/users" className="h-9 px-3 rounded-md border text-sm">Clear</a>
            </form>
          </div>
            ...
            <table className="min-w-[880px] w-full table-fixed text-sm">
              ...
              <tbody className="[&_tr+tr]:border-t">
                {users.map((u) => {
                  ...
                  const created = formatDT(u.created_at);
                  const last = formatDT((u as any).last_sign_in_at);
