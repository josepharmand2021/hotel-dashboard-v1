...
export default function RabAllocationsListPage() {
  ...
  // 👉 role flags (tanpa CanWrite)
  const [readyFlags, setReadyFlags] = useState(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!abort) setCanWrite(!!(json?.isAdmin || json?.isSuper));
      } catch {
        /* ignore */
      } finally {
        if (!abort) setReadyFlags(true);
      }
    })();
    return () => { abort = true; };
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* tombol HANYA untuk admin/super; viewer tidak melihat */}
        {readyFlags && canWrite && (
          <Button asChild>
            <Link href="/finance/rab-allocations/new">New Allocation</Link>
          </Button>
        )}
      </div>
