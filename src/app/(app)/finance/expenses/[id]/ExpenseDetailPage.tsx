...
export default function ExpenseDetailPage({ canWrite }: { canWrite: boolean }) {
  ...
  async function handleSetStatus(s: 'posted'|'draft'|'void') {
    if (!row) return;
    if (!canWrite) return toast.error('Akses baca saja');
    setActing(true);
    try {
        ...
        <div className="flex gap-2">
          ...
          {canWrite && (
            <>
              ...
              {row?.status !== 'posted' && (
                <Button onClick={()=>handleSetStatus('posted')} disabled={acting}>Post</Button>
              )}
              {row?.status === 'posted' && (
                <Button variant="secondary" onClick={()=>handleSetStatus('draft')} disabled={acting}>
                  Set Draft
                </Button>
              )}
