...
export default function CapitalInjectionDetailPage() {
  ...
  // role flag: admin/super bisa nulis
  ...
  // fetch role flags (viewer -> write=false)
  useEffect(() => {
    let abort = false;
    (async () => {
      try {
  ...
  // ===== actions (short-circuit jika !write) =====
  async function onActivate() {
    if (!write) return;
    try { await activatePlan(id); toast.success('Plan di-activate & snapshot dibuat bila belum ada'); fetchAll(); }
  ...
  async function onSnapshot(replace: boolean) {
    if (!write) return;
    try { await snapshotObligations(id, replace); toast.success(replace ? 'Snapshot di-regenerate' : 'Snapshot dibuat'); fetchAll(); }
    catch (e: any) { toast.error(e?.message || 'Gagal snapshot'); }
  }
  ...
  async function saveEditOb() {
    if (!write || !editObId) return;
    const amt = Number(editAmount);
    if (!Number.isFinite(amt) || amt < 0) return toast.error('Nominal tidak valid');
    try { await updateObligation(editObId, Math.round(amt), editReason || undefined); toast.success('Obligation diperbarui'); setEditOpen(false); fetchAll(); }
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="w-full bg-muted rounded-full h-2 mt-1">
                <div
                  className={`h-2 rounded-full ${totals.progress >= 100 ? 'bg-green-500' : 'bg-primary'}`}
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
          {plan.status === 'draft' && (
            <div className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg p-3">
              Plan masih <b>draft</b>. Aktifkan atau klik <b>Generate / Regenerate Snapshot</b> setelah plan aktif
              untuk membuat kewajiban per shareholder berdasarkan ownership snapshot.
            </div>
          )}
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {write && <TableHead className="text-right w-[140px]">Aksi</TableHead>}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {progressRows.map((r) => {
                    ...
                    {progressRows.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={write ? 7 : 6} className="text-center text-muted-foreground py-10">
                          Tidak ada data
                        </TableCell>
                      </TableRow>
                    )}
        ...
        <TabsContent value="contributions" className="space-y-4 mt-4">
          ...
          {/* Form tambah contribution: admin/super only */}
          {write && plan.status === 'active' && (
            <Card>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Status</TableHead>
                      {write && <TableHead className="w-[140px]"></TableHead>}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                      ...
                      <TableRow key={c.id}>
                        ...
                        {write && (
                          <TableCell className="text-right flex justify-end gap-2">
                            ...
                            {c.status !== 'posted' && (
                              <Button size="sm" variant="ghost" className="text-red-600" onClick={() => onDeleteContribution(c.id)}>
                                Hapus
                              </Button>
                            )}
                          </TableCell>
                    ...
                    {contriRows.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={write ? 5 : 4} className="text-center text-muted-foreground py-10">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {write && <TableHead className="w-[90px]"></TableHead>}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      ...
                      {obligationRows.length === 0 && (
                        <TableRow>
                          <TableCell colSpan={write ? 4 : 3} className="text-center text-muted-foreground py-10">
                            Belum ada snapshot
                          </TableCell>
                        </TableRow>
                      )}
      ...
      {/* Dialogs hanya untuk admin/super */}
      {write && (
        <>
