...
export default function GRNListPage() {
  ...
  const COLS = 7; // kolom tabel utama
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        {/* Buat GRN hanya untuk admin/super */}
        {write && (
                  ...
                  <Fragment key={r.id}>
                    <tr className="border-t">
                      ...
                      <td className="px-3 py-2 text-right space-x-2">
                        ...
                        {/* Post hanya untuk admin/super & hanya saat draft */}
                        {write && String(r.status).toLowerCase() === 'draft' && (
                          <Button
                            variant="outline"
                            size="sm"
                                ...
                                <table className="w-full text-sm">
                                  ...
                                  <tbody>
                                    {itemsMap[r.id]!.map((it, idx) => {
                                      ...
                                      const recQ = Number(
                                        it.qty_received ?? it.received_qty ?? it.total_received_qty ?? 0
                                      );
                                      const ovQ = Number(
                                        it.qty_overage ?? it.overage_qty ?? Math.max(recQ - poQ, 0)
                                      );
                                      return (
      ...
      {/* Modal/form hanya mount untuk admin/super */}
      {write && open && <GRNForm onClose={() => { setOpen(false); load(); }} />}
      {write && receiveForId && (
